...
class TicketController extends Controller
{
    ...
    public function assignAdminInCharge(Request $req)
    {
        ...
        $affected = DB::table('job_tickets')
            ...
            ->update(['admin_charge' => $req->staff_id]);

        return redirect()->back();
    }

    public function assignTutor(Request $req, $subjectID, $ticketID)
    {
        ...
        $student = DB::table('students')->where('id', '=', $ticketUID->student_id)->first();
        ...
        $tutorDevices = DB::table('tutor_device_tokens')->where('tutor_id', '=', $tutorID)->distinct()->get(['device_token', 'tutor_id']);
        foreach ($tutorDevices as $rowDeviceToken) {
            ...
            $title = 'JOB-Ticket Assigned';
            $message = 'JOB-Ticket Assigned to Tutor Successfully';
        ...
        $data = [
            "ResponseCode" => "100",
            "message" => "ticket has been Assigned to Tutor successfully!"
        ];
        
        event(new JobTicket($data, $customer->token));
        event(new TutorOffers($data, $tutor->token));

        return redirect()->back()->with('success', 'ticket has been Assigned to Tutor successfully!');
    ...
    public function editTicketTutor($id)
    {
        ...
        if(!$data){
            return redirect()->back()->with('success', 'ticket has been already deleted!');
        }
    ...
    {
        // Cancel all scheduled classes for the ticket
        $class_schedules = DB::table("class_schedules")->where(["ticketID" => $request->id, "status" => "scheduled"])->get();
        foreach ($class_schedules as $class_schedule) {
            DB::table("class_schedules")->where(["id" => $class_schedule->id])->update(["status" => "cancelled"]);
        }
    
        // Update job ticket status
        DB::table("job_tickets")->where("id", $request->id)->update([
            "ticket_tutor_status" => $request->ticket_tutor_status,
            "reason" => $request->reason
        ]);
        
        $job_ticket = DB::table("job_tickets")->where("id", $request->id)->first();
        
        // Send push notifications to tutor devices
        $tutorDevices = DB::table('tutor_device_tokens')->where('tutor_id', '=', $job_ticket->tutor_id)->distinct()->get(['device_token', 'tutor_id']);
        foreach ($tutorDevices as $rowDeviceToken) {
            $push_notification_api = new PushNotificationLibrary();
            $deviceToken = $rowDeviceToken->device_token;
            $title = 'JOB-Ticket Discontinue';
            $message = 'JOB-Ticket has been Discontinue';
        ...
        // Send push notification to parent devices
        $student = DB::table('students')->where('id', '=', $job_ticket->student_id)->first(); // Fetch the first student record
        if ($student) { // Ensure that student exists
            $parent_device_tokens = DB::table('parent_device_tokens')->where('parent_id', '=', $student->customer_id)->distinct()->get(['device_token', 'parent_id']);
            foreach ($parent_device_tokens as $token) {
                $push_notification_api = new PushNotificationLibrary();
                $deviceToken = $token->device_token;
        ...
        return redirect("/TicketList")->with('success', 'Ticket has been Discontinue!');
    }
    ...
    public function TicketList(Request $request)
    {
        ...
        if ($request->filled('cityID')) {
            $query->where('classCity', '=', $request->cityID);
        }
        ...
        $staffs = DB::table('staffs')->where("staffs.status", "Active")->get();
        ...
        return view('job_tickets/ticketList', compact('tickets', 'staffs', 'states', 'subjects', 'cities'));
    }
    ...
    public function TutorApplicationSummary(Request $request)
    {
        $query = DB::table("tutoroffers")
            ->join("products", "tutoroffers.subject_id", "=", "products.id")
            ->join("tutors", "tutoroffers.tutorID", "=", "tutors.id")
            ->leftJoin("states", "tutors.state", "=", "states.id")
            ->leftJoin("cities", "tutors.city", "=", "cities.id")
            ->select(
                'tutoroffers.*', 'products.name as subject', 'tutors.uid', 'tutors.full_name as tutor', 'tutors.email', 'states.name as state_name', 'cities.name as city_name'
            );

        if ($request->filled('stateID')) {
            $query->where('tutors.state', $request->stateID);
        }

        if ($request->filled('subjectID')) {
            $query->where('tutoroffers.subject_id', $request->subjectID);
        }

        if ($request->filled('classType')) {
            $query->where('tutoroffers.class_type', $request->classType);
        }

        if ($request->filled('status')) {
            if ($request->status == 'no-application') {
                $query->where('tutoroffers.status', null);
            } else {
                $query->where('tutoroffers.status', $request->status);
            }
        }

        if ($request->filled('tutorStatus')) {
            $query->where('tutors.status', $request->tutorStatus);
        }

        if ($request->filled('cityID')) {
            $query->where('tutors.city', $request->cityID);
        }

        if ($request->filled('applicationDateFrom')) {
            $query->whereDate('tutoroffers.created_at', '>=', $request->applicationDateFrom);
        }

        if ($request->filled('applicationDateTo')) {
            $query->whereDate('tutoroffers.created_at', '<=', $request->applicationDateTo);
        }

        if ($request->filled('search')) {
            $search = $request->search;
            $query->where(function ($query) use ($search) {
                $query->where('tutors.uid', 'LIKE', '%' . $search . '%')
                    ->orWhere('tutors.full_name', 'LIKE', '%' . $search . '%')
        ...
        $subjects = DB::table('products')->select('id', 'name')->orderBy('name')->get();
        $cities = DB::table('cities')->select('id', 'name')->orderBy('name')->get();

        return view('job_tickets/TutorApplicationSummary', compact('data', 'states', 'subjects', 'cities'));
    }
    ...
    public function addTicket()
    {
        ...
        return view('job_tickets/addTicket', compact('students', 'subjects', 'customers', 'today', 'subjectsPhysical', 'subjectsOnline', 'staffs'));
...
//        dd($currentID);
    ...
    public function viewTicket($id)
    {
        ...
        // Assuming $id is the ticketID being passed
        $tutors = DB::table('tutors')
            ->leftJoin('tutoroffers', function($join) use ($id) {
                $join->on('tutors.id', '=', 'tutoroffers.tutorID')
                     ->where('tutoroffers.ticketID', '=', $id);
            })
            ->whereNull('tutoroffers.ticketID') // Filter out tutors who have sent an offer
            ->where('tutors.status', 'verified') // Only verified tutors
            ->select('tutors.*') // Select fields from tutors table
            ->get();
        ...
        return view('job_tickets/viewTicket', compact('students', 'tutors', 'subjects', 'customers', 'tickets', 'studentDetail', 'tutorOffers',
            'customerPaymentFlag', 'customerCommitmentFeeCheck', 'ticketOffersCheck', 'staffs'));
    ...
    public function editTicket($id)
    {
        ...
        return view('job_tickets/updateTicket',
            compact('students', 'ticketDays',
                'subjects', 'customers', 'singleTicket',
                'singleStudent', 'extraStudents', 'singleCustomer',
                'singleSubject', 'today', 'subjectsPhysical',
                'subjectsOnline'));
...
//  dd($estimate_commission);
    ...
    public function submitTicket(Request $request)
    {
        ...
        for ($i = 0; $i < count($subject); $i++) {
            ...
            if (isset($data['studentFullName'])) {
                for ($j = 0; $j < count($data['studentFullName']); $j++) {
                    if ($data['studentFullName'][$j] != NULL) {
                        ...
                        if ($data['student_ids'][$j] == 'newStudent') {
                            // Retrieve the `student_id` from the database based on specified fields
                            $newStudentRecord = DB::table('students')
                                ->where('full_name', $data['studentFullName'][$j])
                                ->where('gender', $data['studentGender'][$j])
                                ->where('age', $data['age'][$j])
                                ->where('dob', $data['studentDateOfBirth'][$j])
                                ->where('specialNeed', $data['specialNeed'][$j])
                                ->first();
                            
                            $studentID = $newStudentRecord->id; // Capture the student_id for insertion
                        } else {
                            $studentID = $data['student_ids'][$j];   
                        }
            ...
            //     $per_hour_charges = DB::table("categories")->where(["mode" => $modeOfClass, "category_name" => $category_level_subject])->value('tutor_longterm_commission_before_eight_hours');
            //     $per_hour_charges_addition = DB::table("categories")->where(["mode" => $modeOfClass, "category_name" => $category_level_subject])->value('tutor_longterm_commission_after_eight_hours');
            ...
            //     $per_hour_charges =  DB::table("categories")->where(["mode" => $modeOfClass, "category_name" => $category_level_subject])->value('tutor_shorterm_commission');
            ...
            if ($class_term == "Long-Term")
            {
                ...
                if ($modeOfClass == "online")
                {
                    if ($numberOfSessions <= 8) {
                        $estimate_commission += (floatval($per_hour_charges + $extraChargesTutorComission) * $numberOfSessions);
                        $per_class_commission_before_eight_hours += (floatval($per_hour_charges) * $numberOfSessions) / $numberOfSessions;
                        $per_class_commission_after_eight_hours = 0;

                    } else {
                       
                        $estimate_commission += (floatval($per_hour_charges + $extraChargesTutorComission) * 8);
                        $per_class_commission_before_eight_hours += (floatval($per_hour_charges) * 8) / 8;
                        $estimate_commission += (floatval($per_hour_charges_addition + $extraChargesTutorComission) * ($numberOfSessions - 8));
                        $per_class_commission_after_eight_hours += (floatval($per_hour_charges_addition) * ($numberOfSessions - 8)) / ($numberOfSessions - 8);
                ...
                $estimate_commission += (floatval($per_hour_charges + $extraChargesTutorComission) * floatval($data['classFrequency'][$i]) * floatval($data['quantity'][$i]));
                $estimate_after_eight_hours += (floatval($per_hour_charges + $extraChargesTutorComission) * floatval($data['classFrequency'][$i]) * floatval($data['quantity'][$i]));
                // $per_class_commission_before_eight_hours += (floatval($per_hour_charges) * floatval($data['classFrequency'][$i]) * floatval($data['quantity'][$i])) / $data['classFrequency'][$i];
                // $per_class_commission_after_eight_hours += (floatval($per_hour_charges) * floatval($data['classFrequency'][$i]) * floatval($data['quantity'][$i])) / $data['classFrequency'][$i];
                $per_class_commission_before_eight_hours =  $per_hour_charges;
                $per_class_commission_after_eight_hours =  $per_hour_charges;
            }
            ...
            if ($subjectDetail->class_mode == "physical") {
                $tutorCharges = $extraStudentFeeCharges->tutor_physical;
                $jobTicketCharges = $extraStudentFeeCharges->physical_additional_charges;
            } else {
                $tutorCharges = $extraStudentFeeCharges->tutor_online;
                $jobTicketCharges = $extraStudentFeeCharges->online_additional_charges;
            }

            $multiplier = ($count == 0) ? 1 : $count;

            $additionalStudentChargesTutor = $multiplier * $tutorCharges;
            $additionalStudentChargesJobTicket = $multiplier * $jobTicketCharges;
            ...
            if (isset($data["studentFullName"])) {
                $calcPrice += $extraCharges;
            }

            $calcPrice *= $data['classFrequency'][$i];
            $calcPrice *= $data['quantity'][$i];


            $commitmentFeeCheck = DB::table("customer_commitment_fees")->where("customer_id", $customerDetail->id)->first();
            ...
            try {
                ...
                event(new SingleParentDashboard($pusherdata, $customer->token));
            ...
            // Send messages to tutor listings
        ...
        try {
            ...
            event(new TicketCreated($pusherdata));


            event(new JobTicket($pusherdata, $customer->token));
    ...
    public function submitTDuplicateicket(Request $request)
    {
        $data = $request->all();
