         ...
         <div class="nk-content-body">
            ...
            <div class="nk-block">
               <div class="card overflow-hidden">
                  <div class="card-body">
                     <form action="{{url("/financialReport/customer_statement")}}" method="GET">
                     @csrf
                        ...
                        <div class="col-md-4">
                           <div class="form-group">
                              ...
                              <select class="form-control selectpicker" id="account_id" name="account_id" data-live-search="true" data-live-search-style="true" title="Select customer">
                                 ...
                                 @php $customers = DB::table('customers')->join("customer_commitment_fees","customers.id","=","customer_commitment_fees.id")->
                                 where('customers.status', 'active')->get();
                                 @endphp
                                 @foreach($customers as $account)
                                 <option {{Request::get("account_id")==$account->id?"selected":""}} value="{{$account->id}}">{{$account->full_name}}</option>
                                 @endforeach
                              </select>
                           </div>
                        </div>
