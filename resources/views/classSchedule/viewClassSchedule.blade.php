@extends('layouts.main') @section('content')
    ...
    <div class="row view-sindetails">
        <div class="col-md-2 details-item">
            ...
            <p><strong>{{ $studentDetail->uid ?? 'N/A' }}</strong></p>
        </div>
        <div class="col-md-3 details-item">
            <p class="item-title">Fullname</p>
            <p><strong>{{ $studentDetail->full_name ?? 'N/A' }}</strong></p>
        </div>
        <div class="col-md-3 details-item">
            <p class="item-title">Subject</p>
            <p><strong>
                @php
                    $getSubject = DB::table('student_subjects')->where('ticket_id', $tickets->id ?? 0)->first();
                    $getSubjectName = $getSubject ? DB::table('products')->where('id', $getSubject->subject)->first() : null;
                @endphp
                {{ $getSubjectName->name ?? 'N/A' }}
            </strong></p>
        </div>
        <div class="col-md-2 details-item">
            <p class="item-title">Subscribed Duration</p>
            <p><strong>{{ $total_subscribed_hours ?? 0 }} hr(s)</strong></p>
        </div>
        <div class="col-md-2 details-item">
            <p class="item-title">Assigned Duration</p>
            <p><strong>{{ number_format($total_attended_hours ?? 0, 2) }} hr(s)</strong></p>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-4">
            <div class="form-group">
                ...
                <div class="form-control-wrap">
                    <input type="text" readonly class="form-control" value="{{ $tutorDetail->full_name ?? 'N/A' }}"/>
        ...
        <div class="col-lg-4">
            <div class="form-group">
                ...
                <div class="form-control-wrap">
                    <input type="date" name="date" value="{{ $currentDate }}" id="scheduleDate" class="form-control" required>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-4">
            <div class="form-group">
                ...
                <div class="form-control-wrap">
                    <input readonly type="text" class="form-control" value="{{ $subjectDetail->name ?? 'N/A' }}"/>
        ...
        <div class="col-lg-3">
            <div class="form-group">
                ...
                <div class="form-control-wrap">
                    <input readonly type="text" name="totalClasses" value="{{ $tickets->classFrequency ?? 0 }}" class="form-control">
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-3">
            <div class="form-group">
                ...
                <div class="form-control-wrap">
                    <input readonly type="text" name="remaining_classes" value="{{ $tickets->remaining_classes ?? 0 }}" class="form-control">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-footer">
        <input type="submit" class="btn btn-success" value="Add Class Schedule">
                    ...
                    <div class="nk-block">
                        <div class="card">
                          <div class="card-body">
                            <form method="GET" action="{{ route('viewClassSchedules', ['id' => $id]) }}">
                ...
                <input type="submit" class="btn btn-primary" value="Search">
                            ...
                            <div class="row view-sindetails">
                                <div class="col-md-2 details-item">
                                    ...
                                    <p><strong>{{isset($tickets)?$tickets->uid:""}}</strong></p>
                                </div>
                                <div class="col-md-2 details-item">
                                    <p class="item-title">Student</p>
                                    <p><strong>{{isset($studentDetail)?$studentDetail->uid:""}}</strong></p>
                                </div>
                                <div class="col-md-2 details-item">
                                    <p class="item-title">Fullname</p>
                                    <p><strong>{{isset($studentDetail)?$studentDetail->full_name:""}}</strong></p>
                                </div>
                                <div class="col-md-2 details-item">
                                    <p class="item-title">Subject</p>
                                    <p><strong>{{isset($subjectDetail)?$subjectDetail->name:""}}</strong></p>
                                </div>
                                <div class="col-md-2 details-item">
                                    <p class="item-title">Subscribed duration</p>
                                    <p><strong>{{isset($total_subscribed_hours)?$total_subscribed_hours:"00"}} hrs</strong></p>
                                </div>
                                <div class="col-md-2 details-item">
                                    <p class="item-title">Assigned duration</p>
                                    <p><strong>{{isset($total_attended_hours)?number_format($total_attended_hours,2):"00"}} hrs</strong></p>
                                </div>
                            </div>
                            ...
                            <table class="datatable-init table" data-nk-container="table-responsive">
                                ...
                                <tbody>
                                <input style="display:none" type="text" class="class_frequency" value="{{isset($tickets)?$tickets->quantity:""}}">

                              
                                
                                
                                @if(isset($class_schedulesByID))
                                @foreach($class_schedulesByID as $key=>$row)
                                    <tr>
                                        <th>{{$key+1}}</th>
                                        ...
                                        <td>{{ $tutorDetail->full_name ?? 'N/A' }}</td>
                                        <td>
                                            <p class="dtb-vcsstatus">@php $class_attendeds = DB::table('class_attendeds')->where('class_schedule_id','=',$row->id)->where('status','=','attended')->first(); 
                                            $class_data = DB::table('class_attendeds')->where('class_schedule_id','=',$row->id)->where('startTime','!=',NULL)->where('endTime','!=',NULL)->where('status','=','attended')->first();
                                                ...
                                                <div class="modal fade dtable-modal"
                                                    ...
                                                    <div class="modal-dialog modal-xl" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                @php 
                                                                    $startTimeAttended = DB::table('class_attendeds')->where('class_schedule_id','=',$row->id)->where('startTime','!=',NULL)->first(); $endTimeAttended = DB::table('class_attendeds')->where('class_schedule_id','=',$row->id)->where('endTime','!=',NULL)->first();
                                            ...
                                            <div class="modal fade dtable-modal" id="exampleModalAttended{{$row->id}}"
                                                ...
                                                <div class="modal-dialog modal-xl" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                @php
                                                    $startTimeAttended = DB::table('class_attendeds')
                                                        ->where('class_schedule_id', $row->id)
                                                        ->whereNotNull('startTime')
                                                        ->first();
                                                    $endTimeAttended = DB::table('class_attendeds')
                                                        ->where('class_schedule_id', $row->id)
                                                        ->whereNotNull('endTime')
                                                        ->first();
                                                    $getSubject = DB::table('student_subjects')
                                                        ->where('ticket_id', $tickets->id)
                                                        ->first();
                                                    $getSubjectName = DB::table('products')
                                                        ->where('id', $getSubject->subject)
                                                        ->first();
                                                ...
                                                @if (!$startTimeAttended)
                                                    <form method="POST" action="{{ route('submitCheckIn') }}" enctype="multipart/form-data">
                                                        @csrf
                                                        <input type="hidden" name="subjectID" value="{{ $getSubjectName->id }}"/>
                                                        <input type="hidden" name="ticketID" value="{{ $tickets->id }}"/>
                                                        <input type="hidden" name="studentID" value="{{ $studentDetail->id }}"/>
                                                        <input type="hidden" name="classScheduleID" value="{{ $row->id }}"/>
                                                        <input type="hidden" name="csID" value="{{ $row->class_schedule_id }}"/>
                                                        ...
                                                        <div class="row view-sindetails">
                                                            ...
                                                            <div class="col-md-3 details-item">
                                                                <p class="item-title">Subject</p>
                                                                <p><strong>{{ $getSubjectName->name }}</strong></p>
                                                            </div>
                                                            <div class="col-md-2 details-item">
                                                                <p class="item-title">Subscribed Duration</p>
                                                            ...
                                                            <div class="col-md-2 details-item">
                                                                <p class="item-title">Assigned Duration</p>
                                                                <p><strong>{{ number_format($total_attended_hours, 2) }} hr(s)</strong></p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="row g-3 mb-4">
                                                            ...
                                                            <div class="col-lg-4">
                                                                <div class="form-group">
                                                                    ...
                                                                    <input type="date" name="date" readonly value="{{ $newDate }}" class="form-control">
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-4">
                                                                <div class="form-group">
                                                                    <label for="subject" class="form-label">Subject</label>
                                                                    <input type="text" class="form-control" readonly value="{{ $subjectDetail->name }}"/>
                                                                    <input type="hidden" name="subjectID" value="{{ $subjectDetail->id }}"/>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-4">
                                                                <div class="form-group">
                                                                    <label for="start_time" class="form-label">Clock In</label>
                                                                    <input type="time" name="start_time" class="form-control" required>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-4">
                                                                <div class="form-group">
                                                                    <label for="signInProof" class="form-label">Attendance Proof at Clock In</label>
                                                                    <input type="file" name="signInProof" accept="image/png, image/jpeg" class="form-control" required>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <!-- Clock Out Form -->
                                                @if ($startTimeAttended && !$endTimeAttended)
                                                    <form method="POST" action="{{ route('submitCheckOut') }}" enctype="multipart/form-data">
                                                        @csrf
                                                        <input type="hidden" name="id" value="{{ $startTimeAttended->id }}"/>
                                                        <input type="hidden" name="classScheduleID" value="{{ $row->id }}"/>
                                                        <input type="hidden" name="csID" value="{{ $row->class_schedule_id }}"/>
                                                        ...
                                                        <div class="row g-3 mb-4">
                                                            ...
                                                            <div class="col-lg-4">
                                                                <div class="form-group">
                                                                    <label for="end_time" class="form-label">Clock Out</label>
                                                                    <input type="time" name="end_time" value="{{ $startTimeAttended->endTime ?? '' }}" class="form-control" required>
                                                                </div>
                                                            </div>
                                                            <div class="col-lg-4">
                                                                <div class="form-group">
                                                                    <label for="signOutProof" class="form-label">Attendance Proof at Clock Out</label>
                                                                    <input type="file" name="signOutProof" accept="image/png, image/jpeg" class="form-control" required>
                                                                </div>
                                                            </div>
                                                        </div>
                                                                        ...
                                                                        <div class="form-group">
                                                                            ...
                                                                            <div class="form-control-wrap">
                                                                                <input type="text" class="form-control"
                                                                                       value="{{ $tutorDetail->full_name ?? 'N/A' }}"/>
    ...
    <script>
        ...
        function updateEndTimeEdit() {
            var startTimeInput = document.getElementById("startTimeEdit");
            var startTimeValue = startTimeInput.value;
            const duration = parseFloat($(".class_frequency").val());
            var [hours, minutes] = startTimeValue.split(":").map(Number);
            var totalMinutes = hours * 60 + minutes;
            ...
            minutes = totalMinutes % 60;
            var updatedEndTime = (hours < 10 ? "0" : "") + hours + ":" + (minutes < 10 ? "0" : "") + minutes;
            var endTimeInput = document.getElementById("endTimeEdit");
            endTimeInput.value = updatedEndTime;
            console.log("Updated Edit end time:"+updatedEndTime);
        }
    </script>
    <script>
