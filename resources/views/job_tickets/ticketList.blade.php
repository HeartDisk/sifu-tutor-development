                            ...
                            <div class="card-body">
                                <form action="{{ route('TicketList') }}" method="GET">
                                    ...
                                    <div class="row tableper-row">
                                            ...
                                            <div class="input-group input-group-md">
                                                ...
                                                <select class="form-control" id="stateID" name="stateID">
                                                    ...
                                                    @foreach($states as $rowState)
                                                        <option value="{{ $rowState->name }}" {{ old('stateID', request('stateID')) == $rowState->name ? 'selected' : '' }}>{{ $rowState->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group input-group-md">
                                                <label class="input-group-text" for="stateID">City</label>
                                                <select class="form-control" id="stateID" name="cityID">
                                                    <option value="">Select City</option>
                                                    @foreach($cities as $city)
                                                        <option value="{{ $city->name }}" {{ old('stateID', request('cityID')) == $city->name ? 'selected' : '' }}>{{ $city->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group input-group-md">
                                                ...
                                                <select class="form-control" id="status" name="status">
                                                    ...
                                                    <option value="discontinued" {{ old('status', request('status')) == 'discontinued' ? 'selected' : '' }}>Discontinued</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="input-group input-group-md">
                                                <label class="input-group-text" for="staff">Person Incharge</label>
                                                <select class="form-control" id="staff" name="staff">
                                                    <option value="">Select option</option>
                                                    @foreach($staffs as $staff)
                                                        <option value="{{ $staff->id }}" {{ old('staff', request('staff')) == $staff->id ? 'selected' : '' }}>{{ $staff->full_name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="input-group input-group-md">
                                                ...
                                                <input name="search" type="text" class="form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm" placeholder="Ticket Number" value="{{ old('search', request('search')) }}">
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="input-group input-group-md">
                                ...
                                <table class="datatable-init table" data-nk-container="table-responsive">
                                    ...
                                    @endphp
                                        ...
                                        <tr>
                                            <td>{{$numbers++}}</td>
                                            <td>{{$rows->uid}}</td>
                                            <td><p class="tstb-status">{{$rows->ticket_tutor_status==null?"Active":ucfirst($rows->ticket_tutor_status)}}</p></td>
                                            <td>RM{{$rows->totalPrice}}</td>
                                            <td>
                                                @php
                                                    $student_detail = DB::table('students')->where('id','=',$rows->student_id)->first();
                                            ...
                                            <td>
                                                @if(isset($parent_detail) && $parent_detail->full_name)
                                            ...
                                            <td>
                                                ...
                                                @endcan
                                                 ...
                                                 <a class="dtable-cbtn bt-copy dtb-tooltip" dtb-tooltip="Duplicate Ticket" href="{{route('editTicket',$rows->id)}}"><i class="fa fa-copy"></i></a>
                                            ...
                                            <tr>
                                               ...
                                               <td>{{ ucfirst($rows->mode) }}</td>
                                                ...
                                                <td>
                                                    ...
                                                    @if($rows->application_status == 'completed')
                                                        @can("job-ticket-edit")
                                                        <a class="dtable-cbtn bt-edit dtb-tooltip" dtb-tooltip="Edit" href="{{route('editTicketTutor',$rows->id)}}"><i class="fa fa-edit"></i></a>
                                                        @endcan
                                                    @endif

                                                     @if($rows->application_status == 'no-application')
                                                        @can("job-ticket-delete")
                                                            <a class="dtable-cbtn bt-delete dtb-tooltip" dtb-tooltip="Delete" onclick="return confirm('Are you sure you want to delete this Parrent?');" href="{{route('deleteTicket',$rows->id)}}"><i class="fa fa-trash"></i></a>
                                                        @endcan
                                                    @endif
