   ...
   public function incomeStatement(Request $request)
{
    ...
    // Returning the view with the calculated data
    return view('financialReport/incomeStatement', compact(
        'invoices_sum', 'customer_commitment_fee', 'tutor_commitment_fee', 'tutor_payments', 
        'operating_expense', 'provisional_expense', 'staff_cost', 'finance_cost', 
...
public function incomeByProduct(Request $request)
{
    ...
    public function customer_statement(Request $req)
    {
        ...
        $customers = DB::table('customers')->leftjoin("customer_commitment_fees","customers.id","=","customer_commitment_fees.id")->
                                                select("customers.*")->
        ...
        if ($req->has('account_id')) {
            ...
            return view('financialReport.customer_statement', compact('opening_balance', 'totalDebit', 'totalCredit', 'sorted_data','customers'));
        }


        return view('financialReport.customer_statement', compact('customers'));
    }
    ...
    public function expense_voucher_list()
    {
        ...
        leftjoin("chart_accounts", "expense_vouchers.expense_id", "=", "chart_accounts.id")->
        select("expense_vouchers.*", "chart_accounts.name as expense_name")->
        orderBy("id", "desc")->
        get();
    ...
    {
         $bank_vouchers = BankVoucher::orderBy("id","desc")->get();
    ...
    {
        $customer_receive_voucher = BankVoucher::where("bank_vouchers.id", $id)->first();
...
//        dd($request->all());
    ...
    {
        $banks  = ChartOfAccount::join("chart_of_account_sub_categories","chart_accounts.sub_category_id","=","chart_of_account_sub_categories.id")
        ->select("chart_accounts.*")->where("chart_of_account_sub_categories.name","BANK")->get();
       
       
        return view('financialReport.voucher.bank_voucher', compact('banks'));
    }
    ...
    public function bank_statement(Request $req)
    {
        ...
        $from_date = $req->input('from_date', null);  
        $to_date = $req->input('to_date', null); 
        ...
        if (isset($req->account_id)) {
            ...
            $opening_balance = ChartOfAccount::where("name",$req->account_id)->first();
            $opening_balance = $opening_balance->initial_balance;
            ...
            if (!is_null($from_date) && !is_null($to_date)) {
                $credit_data_sum = BankVoucher::where(["bank" => $req->account_id, "type" => "receiving"])
                    ->whereBetween('created_at', [$from_date, $to_date])
                    ->sum("amount");

                $debit_data_sum = BankVoucher::where(["bank" => $req->account_id, "type" => "payment"])
                    ->whereBetween('created_at', [$from_date, $to_date])
                    ->sum("amount");

                $credit_data = BankVoucher::where(["bank" => $req->account_id, "type" => "receiving"])
                    ->whereBetween('created_at', [$from_date, $to_date])
                    ->get();

                $debit_data = BankVoucher::where(["bank" => $req->account_id, "type" => "payment"])
                    ->whereBetween('created_at', [$from_date, $to_date])
                    ->get();
            } else {
            ...
            return view('financialReport.bank_statement', compact('opening_balance',
                'totalDebit', 'totalCredit', 'banks', 'sorted_data'));
        }
        return view('financialReport.bank_statement', compact('banks'));
