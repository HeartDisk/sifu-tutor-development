         ...
         <div class="nk-content-body">
                  ...
                  <div class="nk-block-head-content">
                     ...
                     <nav>
                        <ol class="breadcrumb breadcrumb-arrow mb-0">
                           ...
                           <li class="breadcrumb-item active" aria-current="page">Tutor Ledger</li>
                        </ol>
                     </nav>
                  </div>
               </div>
            ...
            <div class="nk-block">
               <div class="card overflow-hidden">
                  <div class="card-body">
                     <form action="{{url("/financialReport/tutor_statement")}}" method="GET">
                     @csrf
                     <div class="row flex-row align-items-center justify-content-between">
                        <div class="col-md-4">
                           <div class="form-group">
                              <label class="form-label">Select Tutor</label>
                              <select class="form-control selectpicker" id="account_id" name="account_id" data-live-search="true" data-live-search-style="true" title="Select tutor">
                                 <option value="">Select tutor</option>
                                 @foreach($tutors as $tutor)
                                 <option
                                 {{Request::get("account_id")==$tutor->id?"selected":""}} value="{{$tutor->id}}">{{$tutor->full_name}}</option>
                                 @endforeach
                              </select>
                           </div>
                        </div>
                        ...
                        <tbody>
                           ...
                           <tr>
                              ...
                              <td>
                                 @php
                                 //dd($data);
                                 // Ensure $data->amount is treated as a numeric value
                           ...
                           <tr>
                              ...
                              <td></td>
                              <td>Total</td>
                              <td><span id="total_debit">{{number_format($totalDebit,2)}}</span></td>
                              <td><span id="total_credit">{{number_format($totalCredit,2)}}</span></td>
                              <td><span id="total_balance">{{number_format(($totalCredit-$totalDebit)+$opening_balance,2)}}</span></td>
                           </tr>
                        </tbody>
                     </table>
                     @else
                     <p>Please select Tutor</p>
                     @endif
                  </div>
               </div>
            </div>
