                                            ...
                                            <div class="input-group  input-group-md">
                                                ...
                                                <select name="level" class="form-control" id="inputGroupSelect01">
                                                    ...
                                                    @foreach($levels as $level)
                                                        <option {{Request::get("level")==$level->id?"selected":""}} value="{{$level->id}}">{{$level->category_name."(".$level->mode.")"}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="input-group  input-group-md">
                                                ...
                                                <select name="subject" class="form-control" id="inputGroupSelect01">
                                                    ...
                                                    @foreach($subjects as $subject)
                                                        <option {{Request::get("subject")==$subject->id?"selected":""}}  value="{{$subject->id}}">{{$subject->name."-".$subject->category_name."(".$subject->mode.")"}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="input-group  input-group-md">
                                                ...
                                                <select name="state" class="form-control" id="inputGroupSelect01">
                                                    ...
                                                    @foreach($states as $state)
                                                        <option {{Request::get("state")==$state->id?"selected":""}}   value="{{$state->id}}" {{ Request::get('state')}}>{{$state->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="input-group  input-group-md">
                                                ...
                                                <select name="city" class="form-control" id="inputGroupSelect01">
                                                    ...
                                                    @foreach($cities as $city)
                                                        <option {{Request::get("city")==$city->id?"selected":""}}    value="{{$city->id}}" {{ Request::get('state')}}>{{$city->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                            <div class="input-group  input-group-md">
                                                ...
                                                <select name="status" class="form-control" id="inputGroupSelect01">
                                                    ...
                                                    <option {{Request::get("status")=="inactive"?"selected":""}}   value="inactive"> In Active</option>
                                                    <option {{Request::get("status")=="terminated"?"selected":""}}   value="terminated">Terminated</option>
                                                    <option {{Request::get("status")=="resigned"?"selected":""}}   value="resigned">Resigned</option>
                                                    <option {{Request::get("status")=="unverified"?"selected":""}}   value="unverified">Unverified</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-md-2">
                                ...
                                <table class="datatable-init table" data-nk-container="table-responsive">
                                    ...
                                    @foreach($tutors as $rowTutor)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('viewTutor',$rowTutor->id)}}" class="dtable-cbtn bt-view dtb-tooltip" dtb-tooltip="Details"><span class="fa fa-eye"></span></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    </tbody>
